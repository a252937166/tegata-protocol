...
| TEGATA attestor key | signing verified evidence into `SettlementAnchor` | The single demo attestor is a disclosed trust edge. It can only anchor evidence that carries a valid EIP-712 signature over the exact struct; anyone re-running the verifier detects an anchor that contradicts the raw evidence. Production path: threshold/institutional attestors, multisig + timelock admin. |
